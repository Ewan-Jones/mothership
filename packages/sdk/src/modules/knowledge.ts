...
export class KnowledgeBaseApi extends BaseApi {
  ...
  async uploadResources(params: { id: string }, formData: FormData): Promise<ApiResult<UploadKnowledgeResourcesResponse>> {
    return this.upload<UploadKnowledgeResourcesResponse>("/web/knowledgeBases/:id/resources/upload", formData, { params });
  }
  async importUrl(params: { id: string }, body: { url: string; sourceName?: string }): Promise<ApiResult<ImportKnowledgeUrlResponse>> {
  ...
  async deleteResource(params: { id: string; resourceId: string }): Promise<ApiResult<DeleteKnowledgeResourceResponse>> {
