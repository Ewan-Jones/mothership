...
export function ToolCallGroup({
    ...
    const running = entries.filter(
        (e) => e.toolCall.status === "running",
    ).length;
    const complete = entries.filter(
        (e) => e.toolCall.status === "complete",
    ).length;
    ...
    return (
        <div>
            <div
                ...
                <span
                    className={cn(
                        "w-4 flex-shrink-0 text-center text-[11px] font-bold",
                        toolInfo.color,
                    )}>
                    {toolInfo.icon}
                </span>
                ...
                {/* 状态 pill */}
                <span
                    className={cn("tool-status-pill text-[9px]", status.pill)}>
                    {status.label}
                </span>
            ...
            <div
                ...
                <div
                    ref={detailRef}
                    className="border-t border-border/50 bg-surface-1/30">
...
function getDescription(tool: ToolCallData): string {
    ...
    if (tool.title) {
        return tool.title.replace(
            /^(Bash|Edit|Read|Write|Grep|Glob|WebFetch|WebSearch|Task)\s*:\s*/,
            "",
        );
    }
    return "";
