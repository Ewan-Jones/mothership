...
- [x] 创建 `src/routes/web/files.ts` 文件，包含完整路由实现
...
- [x] 实现 POST `/:sessionId/files/*` 上传路由
  - 位置: 读取路由之后
  - 逻辑:
    ```typescript
...
- [x] 实现 DELETE `/:sessionId/files/*` 删除路由
  - 位置: 写入路由之后
  - 逻辑:
    ```typescript
...
- [x] 在 `web/src/types/index.ts` 末尾追加文件相关类型定义
...
- [x] 在 `web/src/api/client.ts` 文件中追加文件操作 API 函数
...
- [x] 在 `web/src/api/client.ts` 顶部导入区域追加类型导入
...
- [x] 创建 `web/src/components/FilePickerDialog.tsx` 文件骨架
...
- [x] 在 `web/src/lib/types.ts` 中扩展 `ChatInputMessage` 类型
...
- [x] 在 `web/components/chat/ChatInput.tsx` 中新增 `sessionId` 和 `onFileSelect` Props
...
- [x] 在 `web/components/chat/ChatInput.tsx` 中新增文件选择相关状态
...
- [x] 修改 `handleSubmit` 回调以包含 attachments
...
- [x] 在 JSX 中添加 FilePickerDialog 渲染和 @ 按钮
...
- [x] 在附件预览区域（images 预览之后）显示已选中的文件附件
...
- [x] 验证 ChatInput Props 新增 sessionId
