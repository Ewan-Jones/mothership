...
- **ACP 协议支持**：通过 WebSocket 与 acp-link 代理通信，实现远程 Agent 控制和事件流转发
...
# 前端开发（Vite dev server，独立进程，代理到后端）
bun run dev:web
...
| 方向 | 间隔 | 说明 |
|------|------|------|
| 服务器 → acp-link | 20s | `keep_alive` 数据帧，防止反向代理超时断开 |
...
2. **`web/components/config/StatusBadge.tsx`**（配置页状态）
   ...
   - `primary` → 主模型, `subagent` → 子代理, `all` → 全部
...
2. **前端构建后路径错误**：Vite base 设为 `/code/`，部署时需确保反向代理匹配
3. **配置写入竞争**：多请求同时修改配置可能损坏，`services/config.ts` 有锁机制但非分布式
4. **WebSocket 断连**：反向代理 timeout 需 > 30s（Bun idleTimeout 默认）
