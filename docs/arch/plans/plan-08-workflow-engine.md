# Plan 08：@fenix/workflow-engine 独立包实现
...
| 与 @fenix/core 关系 | workflow-engine 独立运作，不依赖 core。AgentNode 通过 Transport 接口与已有 Agent 通信，不负责启动新实例 |
...
- 从根目录 `import type { WorkflowDef, DAGEvent, NodeDef, DAGSnapshot, RunSummary } from "@fenix/workflow-engine"` 可解析
