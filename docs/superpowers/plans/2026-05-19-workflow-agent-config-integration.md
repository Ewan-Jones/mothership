...
```typescript
import type { WorkflowEngine, AgentResolvedConfig } from "@fenix/workflow-engine";
import { createWorkflowEngine } from "@fenix/workflow-engine";
import type { Transport } from "@fenix/workflow-engine";
...
注意：需要从 `@fenix/workflow-engine` 导出 `AgentResolvedConfig` 类型。在 `packages/workflow-engine/src/index.ts` 中检查并添加 re-export：
