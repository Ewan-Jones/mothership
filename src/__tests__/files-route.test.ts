...
import { storeCreateEnvironment, storeCreateSession, storeReset } from "../store";
...
describe("Files Route", () => {
  ...
  beforeEach(async () => {
    ...
    const environment = storeCreateEnvironment({
      secret: `test-secret-${crypto.randomUUID()}`,
      userId: "test-user",
      workspacePath: workspaceDir,
      status: "active",
    });
    const session = storeCreateSession({ environmentId: environment.id, idPrefix: "session_" });
  ...
  test("GET /:sessionId/user — falls back to the user's environment when session is missing", async () => {
    await writeFile(join(workspaceDir, "user", "hello.txt"), "Hello World");
  ...
  test(".scheduled-runs path can be listed", async () => {
    await mkdir(join(workspaceDir, ".scheduled-runs", "task_1", "run_1"), { recursive: true });
    await writeFile(join(workspaceDir, ".scheduled-runs", "task_1", "run_1", "report.md"), "# Report");

    const res = await app.request(`/web/sessions/${sessionId}/user?path=.scheduled-runs/task_1/run_1`);
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  test(".scheduled-runs file can be read", async () => {
    ...
    const res = await app.request(`/web/sessions/${sessionId}/user/.scheduled-runs/task_1/run_1/report.md`);
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  test("path traversal is blocked for workspace root and user root", async () => {
    const listRes = await app.request(`/web/sessions/${sessionId}/user?path=../../../etc`);
    expect(listRes.status).toBe(404);

    const putRes = await app.request(`/web/sessions/${sessionId}/user/../../../etc/evil.txt`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  test("workspace-root write requests are rejected", async () => {
    const putRes = await app.request(`/web/sessions/${sessionId}/user/.scheduled-runs/task_1/run_1/report.md`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ content: "hack" }),
    });
