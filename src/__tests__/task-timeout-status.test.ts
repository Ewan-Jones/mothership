...
async function ensureTeam() {
  ...
  if (existingUser.length === 0) {
    await db.insert(user).values({ id: TEST_USER_ID, name: "Timeout Status", email: "timeout-status@rcs.local", emailVerified: false, createdAt: now, updatedAt: now });
  }
  const [created] = await db.insert(team).values({ name: "Timeout Status Team", slug: TEST_TEAM_SLUG, createdBy: TEST_USER_ID }).returning();
...
async function insertTask(suffix: string) {
  const [row] = await db.insert(scheduledTask).values({
    ...
    name: `to_${suffix}_${Date.now()}`, description: null, cron: "* * * * *", timezone: null,
