...
const mockGlobalSkills: any[] = [{ id: "s1", name: "global-skill", userId: "u1" }];
const mockAgentSkills: any[] = [
  { id: "s1", name: "global-skill", userId: "u1" },
  { id: "s2", name: "agent-skill", userId: "u1" },
];
const mockAgentConfig: any = { id: "ac1", name: "coder", userId: "u1" };

// mock.module 路径相对于测试文件（src/__tests__/），需与 aggregate.ts 的 import 解析到同一绝对路径
// aggregate.ts: import { db } from "../../db" → src/db
// 测试文件: "../db" → src/db
...
/**
 * Drizzle ORM 的 where() 返回 thenable query builder（带 .limit() 等方法）。
 * mock where 返回 thenable 对象：直接 await 得到 result，调用 .limit() 也得到 result。
 */
...
describe("getAgentFullConfig", () => {
  ...
  // agentConfigId 存在时，并行拉取 providers/mcpServers/agentConfig，再拉 skills
  test("returns agentConfig and skills when agentConfigId is provided", async () => {
    ...
    // 并行阶段 3 查询 + skills 查询 = 4 次 select
    expect(mockDbSelect).toHaveBeenCalledTimes(4);
  });

  // agentConfigId 指向不存在的记录时，回退到全局 skills
  test("falls back to global skills when agentConfigId not found", async () => {
    let callIndex = 0;
    const results = [mockProviders, mockMcpServers, [], mockGlobalSkills];
