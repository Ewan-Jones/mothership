...
const app = new Elysia({ name: "web-s3-files", prefix: "/web/s3" })
  .use(authGuardPlugin)
...
// 列出会话文件
app.get("/files", async ({ query, error }) => {
...
// 获取下载 presigned URL
app.get("/files/presign", async ({ query, error }) => {
  const q = query as Record<string, string | undefined>;
  const sessionId = q.sessionId;
  const key = q.key;
  if (!sessionId || !key) return error(400, { error: { type: "validation_error", message: "sessionId and key are required" } });
...
// 获取上传 presigned URL
app.post("/files/presign", async ({ body, error }) => {
  const b = body as S3PresignPutBody;
  if (!b.sessionId || !b.key || !b.contentType) {
    return error(400, { error: { type: "validation_error", message: "sessionId, key and contentType are required" } });
  }
...
app.post("/files/upload", async ({ query, request, error }) => {
  const q = query as Record<string, string | undefined>;
  const sessionId = q.sessionId;
  if (!sessionId) return error(400, { error: { type: "validation_error", message: "sessionId query param is required" } });
...
// 删除文件
app.delete("/files", async ({ body, error }) => {
